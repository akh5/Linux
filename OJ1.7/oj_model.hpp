...
class OjModel
{
    public:
        ...
        bool GetTitle(const string& id,string& title,TestQues& ques)
        {
          ...
          ques=iter->second;
          int ret = FileOpen::ReadDataFromFile(TitlePath(iter->second._path),title);
          if(ret==-1)
          {
            LOG(ERROR,"Read title failed")<<endl;
            return false;
          }
          return true;
        }
    ...
    private:
        ...
        string TitlePath(const string ques_path)
        {
          return ques_path+"title.txt";
        }
